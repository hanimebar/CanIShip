...
/**
 * Auth config supplied by the user to allow CanIShip to test protected pages.
 *
 * SECURITY: Only test/staging credentials should be used. The password is
 * stored temporarily in audit_jobs.auth_config and cleared from the DB
 * immediately after the worker claims the job (before any audit runs).
 *
 * SQL migration required:
 *   ALTER TABLE audit_jobs ADD COLUMN IF NOT EXISTS auth_config JSONB;
 */
export type AuthConfig =
  | {
      type: 'form'
      login_url?: string          // defaults to /login relative to the audited URL
      email: string
      password: string
      email_selector?: string     // CSS selector — auto-detected if omitted
      password_selector?: string  // CSS selector — auto-detected if omitted
      submit_selector?: string    // CSS selector — auto-detected if omitted
      success_selector?: string   // element present after successful login
      success_url_pattern?: string // URL substring present after successful login
    }
  | {
      type: 'cookie'
      cookies: Array<{
...
/** One step inside a user-defined flow, as executed by the flow runner. */
export type FlowStepResult = {
  ...
  details: string
  screenshot_before?: string   // storage path
  screenshot_after?: string    // storage path
}

/** Result of running a single user-defined flow. */
export type FlowExecutionResult = {
  flow_text: string
  overall_status: 'passed' | 'failed' | 'partial' | 'skipped'
...
export type ClaudeReport = {
  ...
  top_5_fixes: Fix[]
  // Added post-analysis by the pipeline — not generated by Claude
  flow_results?: FlowExecutionResult[]
  pages_audited?: number
  privacy_score?: number
  active_security_score?: number
  auth_hardening_score?: number
}
