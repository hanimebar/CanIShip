...
- SCORING RULE: If a test runner (Lighthouse, axe-core) was unavailable due to infrastructure errors, you CANNOT penalise the app for those missing results. Score based only on what was verified. An app with zero bugs, strong security, fast load time, and only runner-unavailable warnings should score 90+.
- SCORING RULE: "Test layer unavailable" is a warning, not a defect. It belongs in warnings[], never in critical_bugs[], ux_issues[], or security_flags[]. It must never reduce the score below what the verified checks justify.
